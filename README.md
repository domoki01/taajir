...
## إعادة البناء بـ Laravel

يجري إعادة بناء الموقع بـ PHP / Laravel في مجلد [`laravel/`](./laravel): نفس
الروابط ونفس الشاشات ونفس القواعد، مع MySQL بدل Firestore، وبقاء Firebase لشيء
واحد فقط هو تسجيل الدخول. الخطة كاملة في
[`docs/laravel-port.md`](./docs/laravel-port.md).

تطبيق Next.js يبقى هو الموقع الحيّ إلى أن تكتمل المراحل، ولهذا يعيش المشروعان
جنباً إلى جنب في نفس المستودع. لا شيء في `laravel/` يستورد من `src/`؛ ألوان
التصميم والمفردات منقولة يدوياً ومحميّة باختبارات.
